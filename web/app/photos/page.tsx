...
export default function PhotosPage() {
  ...
  return (
    <div className="p-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Unknown Visitor Gallery</h1>
      ...
      <div className="mb-6 flex items-center gap-2">
        ...
        <div className="ml-auto text-sm text-gray-600">
          {filteredEvents.length} {filteredEvents.length === 1 ? 'photo' : 'photos'}
        </div>
      </div>
      ...
      {filteredEvents.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-12 text-center">
          <p className="text-gray-500">No photos available for the selected time period.</p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-medium text-orange-600">Unknown</span>
                  <span
                    className={`text-xs font-medium ${
                      event.confidence > 0.5 ? 'text-yellow-600' : 'text-red-600'
                    }`}
                  >
                    {formatConfidence(event.confidence)}
                  </span>
                </div>
                <p className="text-xs text-gray-600">{formatDate(event.occurred_at)}</p>
              </div>
            </div>
          ))}
        </div>
              ...
              <div className="w-full md:w-80 p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-orange-600 font-semibold mt-1">Unknown Visitor</p>
                  </div>

                  <div>
                    <label className="text-sm font-medium text-gray-600">Detection Time</label>
                    <p className="text-gray-900 mt-1">{formatDate(selectedPhoto.occurred_at)}</p>
                  </div>
                  ...
                  <div>
                    ...
                    <p
                      className={`font-semibold mt-1 ${
                        selectedPhoto.confidence > 0.5 ? 'text-yellow-600' : 'text-red-600'
                      }`}
                    >
                  ...
                  <div>
                    ...
                    <p className="text-gray-900 mt-1">#{selectedPhoto.event_id}</p>
                  </div>

                  <div className="pt-4 border-t border-gray-200">
                    <p className="text-xs text-gray-500">
                      This photo was captured when an unregistered person was detected. You can
                      register them as a user from the Users page.
                    </p>
                  </div>
                </div>
              </div>
