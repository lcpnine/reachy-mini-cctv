...
import { User, getUsers, registerUser, deleteUser } from '@/lib/api';
    ...
    <div className="p-8">
      ...
      {users.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {users.map((user) => (
            <div key={user.user_id} className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition-shadow">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="text-xl font-semibold text-gray-900">{user.name}</h3>
                  <p className="text-sm text-gray-600 mt-2">ID: {user.user_id}</p>
                  <p className="text-sm text-gray-600">Registered: {formatDate(user.created_at)}</p>
                </div>
